...
class DockerInstaller {
  ...
  /**
   * Clean up old Docker containers and images for this project
   */
  async cleanupOldBuilds() {
    const chalk = require('chalk');
    const { execSync, spawn } = require('child_process');
    
    console.log(chalk.blue('🧹 Cleaning up old Docker builds...\n'));
    ...
    try {
      ...
      console.log(chalk.gray(`   Stopping and removing containers: ${composeCommand} down --volumes --remove-orphans`));
      ...
      await new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
          // Exit code 0 is success, but also accept if containers didn't exist (might return non-zero)
          resolve();
        });
      ...
      // Remove all scanner-map images (all tags and untagged)
      try {
        console.log(chalk.gray('   Removing all scanner-map images...'));
        
        // First, find all scanner-map images (by repository name)
        // This will catch all tags: scanner-map:latest, scanner-map:v1.0, scanner-map_scanner-map:latest, etc.
        // Docker Compose may prefix images with project name, so check both patterns
        let imagesOutput = '';
        try {
          // Get all images and filter for scanner-map (handles both scanner-map:tag and scanner-map_scanner-map:tag)
          imagesOutput = execSync('docker images --format "{{.Repository}}:{{.Tag}}\t{{.ID}}"', {
            cwd: this.projectRoot,
            stdio: 'pipe',
            encoding: 'utf8'
          });
        } catch (err) {
          // No images found or error, which is fine
        }
        
        if (imagesOutput && imagesOutput.trim()) {
          const imageLines = imagesOutput.trim().split('\n').filter(line => line.trim());
          const scannerMapImages = [];
          ...
          for (const line of imageLines) {
            ...
            // Match scanner-map images (with or without project prefix)
            // Examples: scanner-map:latest, scanner-map_scanner-map:latest, scanner-map:v1.0
            if (imageTag && (imageTag.startsWith('scanner-map:') || imageTag.startsWith('scanner-map_scanner-map:'))) {
              scannerMapImages.push({ tag: imageTag, id: imageId });
            }
          }
          
          // Remove all found scanner-map images
          for (const img of scannerMapImages) {
            try {
              // Try to remove by tag first (force removal even if in use)
              execSync(`docker rmi -f "${img.tag}" 2>&1`, {
                cwd: this.projectRoot,
                stdio: 'pipe',
                encoding: 'utf8'
              });
              console.log(chalk.gray(`     Removed: ${img.tag}`));
            } catch (rmErr) {
              // If tag removal fails (e.g., image in use), try by ID
              if (img.id) {
                try {
                  execSync(`docker rmi -f "${img.id}" 2>&1`, {
                    cwd: this.projectRoot,
                    stdio: 'pipe',
                    encoding: 'utf8'
                  });
                  console.log(chalk.gray(`     Removed: ${img.id} (by ID)`));
                } catch (idErr) {
                  // Image might be in use by a running container or already removed
                  // Try to stop and remove any containers using this image first
                  try {
                    const containers = execSync(`docker ps -a --filter "ancestor=${img.id}" --format "{{.ID}}"`, {
                      cwd: this.projectRoot,
                      stdio: 'pipe',
                      encoding: 'utf8'
                    });
                    if (containers && containers.trim()) {
                      ...
                      for (const containerId of containerIds) {
                        try {
                          execSync(`docker rm -f "${containerId}" 2>&1`, {
                            cwd: this.projectRoot,
                            stdio: 'pipe',
                            encoding: 'utf8'
                          });
                      ...
                      // Try removing image again after removing containers
                      try {
                        execSync(`docker rmi -f "${img.id}" 2>&1`, {
                          cwd: this.projectRoot,
                          stdio: 'pipe',
                        ...
                        console.log(chalk.gray(`     Removed: ${img.id} (after container cleanup)`));
                      } catch (retryErr) {
                        // Still can't remove, might be in use elsewhere
                        console.log(chalk.yellow(`     ⚠ Could not remove ${img.tag} - may be in use`));
                      }
                    }
                  } catch (containerCheckErr) {
                    // Couldn't check containers, continue
                  }
                }
              }
            }
        ...
        // Also check for any untagged/dangling scanner-map images by ID
        // These might exist if images were removed but not fully cleaned
        try {
          const allImages = execSync('docker images --filter "dangling=true" --format "{{.ID}}"', {
            cwd: this.projectRoot,
            stdio: 'pipe',
            encoding: 'utf8'
          });
          
          // Check if any dangling images are from scanner-map builds
          // We can't easily identify them, but docker builder prune will handle them
        } catch (err) {
          // No dangling images or error checking, continue
        }
      } catch (err) {
        // Overall image removal had issues, but continue
        console.log(chalk.yellow(`     ⚠ Some images might still exist: ${err.message}`));
      }
      
      // Clean up dangling build cache (optional - comment out if you want to keep cache)
      try {
        console.log(chalk.gray('   Cleaning up build cache...'));
        execSync('docker builder prune -f', { 
          cwd: this.projectRoot,
          stdio: 'pipe'
        });
      } catch (err) {
        // Builder prune might not be available, ignore
      }
      
      console.log(chalk.green('   ✓ Cleanup complete\n'));
    } catch (err) {
      console.log(chalk.yellow(`   ⚠ Cleanup had some issues (continuing anyway): ${err.message}\n`));
    }
  }
  ...
  async startServices() {
    ...
    try {
      ...
      try {
        ...
        // Add --build flag to force rebuild of images
        const spawnArgs = useDockerCompose 
          ? ['up', '-d', '--build']  // docker-compose is a single command
          : ['compose', 'up', '-d', '--build'];  // docker compose needs 'compose' as first arg, --build forces rebuild
        const spawnCmd = useDockerCompose ? 'docker-compose' : 'docker';
        
        console.log(chalk.gray(`   Building and starting containers: ${composeCommand} up -d --build`));
