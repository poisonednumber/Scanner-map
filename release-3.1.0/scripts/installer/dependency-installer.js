...
class DependencyInstaller {
   ...
   * Improved detection for Windows Docker Desktop
   */
  isDockerDaemonRunning() {
    // First try the standard docker ps command
    try {
      execSync('docker ps', { stdio: 'ignore', timeout: 5000 });
      return true;
    } catch (err) {
      // On Windows, check if Docker Desktop process is running
      if (this.isWindows) {
        try {
          // Check for Docker Desktop process
          const result = execSync('tasklist /FI "IMAGENAME eq Docker Desktop.exe" /FO CSV /NH', { 
            encoding: 'utf8',
            stdio: ['ignore', 'pipe', 'ignore'],
            timeout: 3000
          });
          if (result && result.includes('Docker Desktop.exe')) {
            // Docker Desktop is running, try docker info as a more lenient check
            try {
              execSync('docker info', { stdio: 'ignore', timeout: 5000 });
              return true;
            } catch (infoErr) {
              // Docker Desktop running but daemon might not be ready
              // Check for Docker named pipe (Windows)
              const dockerPipe = '\\\\.\\pipe\\docker_engine';
              try {
                if (fs.existsSync && fs.existsSync(dockerPipe)) {
                  return true;
                }
              } catch (pipeErr) {
                // Ignore pipe check errors
              }
            }
          }
        } catch (tasklistErr) {
