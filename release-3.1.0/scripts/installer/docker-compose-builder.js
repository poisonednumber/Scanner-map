...
class DockerComposeBuilder {
  ...
  async build(services) {
    ...
    const compose = {
      name: 'scanner-map', // Explicit project name to avoid directory-based prefixes
      services: {
        'scanner-map': {
          build: {
            context: '.',
    ...
    if (shouldStartICADContainer) {
      ...
      // Add GPU support if available
      if (gpuConfig) {
        compose.services['icad-transcribe'].deploy = gpuConfig.deploy;
      }
...
`;
   ...
   * @param {boolean} enableGPU - Whether to enable GPU support
   * @returns {Promise<Object>} Result with success status
   */
  async addService(serviceName, enableGPU = false) {
    try {
      // Check if scanner-map is already running in Docker
      const { execSync } = require('child_process');
      let scannerMapRunning = false;
      let existingComposeProject = 'scanner-map';
      
      try {
        // Check if scanner-map container exists and is running
        const containerCheck = execSync('docker ps --filter "name=scanner-map" --format "{{.Names}}"', {
          encoding: 'utf8',
          stdio: 'pipe',
          timeout: 2000
        });
        if (containerCheck && containerCheck.trim().includes('scanner-map')) {
          scannerMapRunning = true;
          // Try to detect the compose project name from the container
          try {
            const inspect = execSync('docker inspect scanner-map --format "{{index .Config.Labels \"com.docker.compose.project\"}}"', {
              encoding: 'utf8',
              stdio: 'pipe',
              timeout: 2000
            });
            if (inspect && inspect.trim()) {
              existingComposeProject = inspect.trim();
            }
          } catch (err) {
      ...
      if (await fs.pathExists(this.composePath)) {
        ...
        // Ensure project name matches existing setup
        if (compose.name) {
          existingComposeProject = compose.name;
        }
      } else if (scannerMapRunning) {
        // Scanner-map is running but no compose file found - this shouldn't happen normally
        // But we'll create one that matches the running container
        compose = {
          name: existingComposeProject,
          services: {
            'scanner-map': {
              container_name: 'scanner-map',
              restart: 'unless-stopped',
        ...
        // Create new compose file with base structure
        compose = {
          name: 'scanner-map',
          services: {
            'scanner-map': {
      ...
      compose.name = existingComposeProject;

      // Ensure networks exist
      if (!compose.networks) {
        compose.networks = { 'scanner-network': { driver: 'bridge' } };
      }
      if (!compose.networks['scanner-network']) {
        compose.networks['scanner-network'] = { driver: 'bridge' };
      }
      ...
      // Add service based on name
      if (serviceName === 'ollama') {
        compose.services.ollama = {
          image: 'ollama/ollama:latest',
          container_name: 'ollama',
          restart: 'unless-stopped',
          volumes: ['./appdata/ollama:/root/.ollama'],
          ports: ['11434:11434'],
          networks: ['scanner-network'],
        ...
        // Add to depends_on if scanner-map exists
        if (compose.services['scanner-map']) {
          if (!compose.services['scanner-map'].depends_on) {
            compose.services['scanner-map'].depends_on = [];
          }
          if (!compose.services['scanner-map'].depends_on.includes('ollama')) {
            compose.services['scanner-map'].depends_on.push('ollama');
          }
        }
      } else if (serviceName === 'icad-transcribe') {
        compose.services['icad-transcribe'] = {
          image: 'thegreatcodeholio/icad_transcribe:1.0',
          container_name: 'icad-transcribe',
          restart: 'unless-stopped',
          ...
          networks: ['scanner-network'],
          environment: [`TZ=${process.env.TIMEZONE || 'America/New_York'}`]
        };
        if (gpuConfig) {
          compose.services['icad-transcribe'].deploy = gpuConfig.deploy;
        }
        ...
        if (compose.services['scanner-map']) {
          ...
          if (!compose.services['scanner-map'].depends_on.includes('icad-transcribe')) {
            compose.services['scanner-map'].depends_on.push('icad-transcribe');
          }
        }
      } else {
      ...
      await fs.writeFile(this.composePath, yamlContent, 'utf8');
