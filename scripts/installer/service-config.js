...
class ServiceConfig {
   ...
   * @param {Object} sdrDevice - Optional SDR device configuration. If not provided, will auto-detect.
   * @returns {Promise<Object>} Configuration result with apiKey if generated
   */
  async configureTrunkRecorder(enabled, installationType = 'docker', apiKey = null, sdrDevice = null) {
    ...
    // Auto-detect SDR device if not provided
    let deviceConfig = sdrDevice;
    if (!deviceConfig) {
      try {
        const SDRDetector = require('./sdr-detector');
        const detector = new SDRDetector();
        const recommendedDevice = await detector.getRecommendedDevice();
        deviceConfig = recommendedDevice;
        console.log(`[SDR] Auto-detected SDR device: ${recommendedDevice.name} (${recommendedDevice.driver}, ${recommendedDevice.device})`);
      } catch (err) {
        console.warn(`[SDR] Could not auto-detect SDR device: ${err.message}`);
        // Fallback to default RTL-SDR configuration
        deviceConfig = {
          driver: 'osmosdr',
          device: 'rtl=0',
          deviceString: 'rtl=0'
    ...
    if (shouldUpdate) {
      try {
        ...
        if (existing.sources && Array.isArray(existing.sources)) {
          existing.sources.forEach((source, index) => {
            // Only migrate if driver is missing (preserve existing good config)
            if (source.type && !source.driver) {
              // Convert type to driver format
              if (source.type === 'rtl_sdr' || source.type === 'rtl') {
                source.driver = 'osmosdr';
                source.device = source.device !== undefined ? (source.device.match(/^rtl=/) ? source.device : `rtl=${source.device}`) : `rtl=${index}`;
              } else if (source.type === 'hackrf') {
                source.driver = 'osmosdr';
                source.device = source.device !== undefined ? (source.device.match(/^hackrf=/) ? source.device : `hackrf=${source.device}`) : `hackrf=${index}`;
              } else if (source.type === 'bladerf') {
                source.driver = 'bladerf';
                source.device = source.device !== undefined ? (source.device.match(/^bladerf=/) ? source.device : `bladerf=${source.device}`) : `bladerf=${index}`;
              } else if (source.type === 'airspy') {
                source.driver = 'osmosdr';
                source.device = source.device !== undefined ? (source.device.match(/^airspy=/) ? source.device : `airspy=${source.device}`) : `airspy=${index}`;
              } else {
                // Unknown type, try to use as driver name
                source.driver = source.type;
              }
              delete source.type;
            }
            // Ensure required fields exist (don't overwrite if already set)
            if (!source.gain && source.gain !== 0) source.gain = 30;
            if (source.error === undefined) source.error = 0;
            if (!source.digitalRecorders) source.digitalRecorders = 4;
          });
        } else if (!existing.sources || existing.sources.length === 0) {
          // No sources found, add default source with auto-detected device
          existing.sources = [{
            driver: deviceConfig.driver || 'osmosdr',
            device: deviceConfig.device || deviceConfig.deviceString || 'rtl=0',
            center: 850000000,
